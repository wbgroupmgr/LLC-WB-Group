...
class GLAuditor:
    ...
    def _check_escrow_balance(self) -> List[Dict[str, Any]]:
        ...
        Forensic: identifies orphan entries (srcTID == tID, no double-entry pair)
        as the primary suspects and finds the minimal set that explains the balance.
        '''
        ...
        # ── Forensic: classify each escrow row ───────────────────────────
        # Orphan = direct-posted entry (srcTID == tID or srcTID absent)
        # Paired = generated by toDoubleEntry (srcTID differs from tID)
        ...
        # Try to find the minimal subset of suspects that exactly explains the diff
        # Signed amounts: Debit = +, Credit = −
        def signed_amt(r):
            return _amt(r) if _is_debit(r) else -_amt(r)

        suspects = _subset_summing_to(orphans, balance, signed=True)
        if suspects is None:
            # Orphans don't explain it alone — try full set
            suspects = _subset_summing_to(escrow_rows, balance, signed=True)
        if suspects is None:
            suspects = orphans if orphans else escrow_rows  # fallback

        affected = sorted([_fmt(r) for r in suspects],
                          key=lambda x: abs(float(x.get('amt') or 0)), reverse=True)

        if balance > 0:
            direction = (f"Debits ({d:.2f}) exceed Credits ({c:.2f}) by {balance:.2f}. "
                         "Missing: a Credit to Acct.Cash.Escrow for the unmatched line items.")
        else:
            direction = (f"Credits ({c:.2f}) exceed Debits ({d:.2f}) by {abs(balance):.2f}. "
                         "Missing: a Debit to Acct.Cash.Escrow (cash-in from Bank or Owner contribution).")

        orphan_note = ''
        if orphans:
            orphan_tids = ', '.join(r.get('tID', '') for r in orphans[:5])
            orphan_note = (
                f"\n\nORPHAN ENTRIES DETECTED ({len(orphans)}): "
                "rows where srcTID = tID indicate a direct-posted entry that was NOT "
                "generated by toDoubleEntry. These have no automatic counterpart.\n"
                f"  Orphan tIDs: {orphan_tids}"
            )
        ...
        return [{
            ...
            'description': (
                ...
                + direction
                + orphan_note
                + "\n\nAffected records below are the minimal set explaining the imbalance. "
                "tDB/tID trace back to the source DB record."
            ),
            'affected': affected,
            'correction': {
                'type': 'manual', 'auto_apply': False,
                'description': (
                    "For each orphan entry: verify it belongs here. "
                    "If it duplicates a PropAgent entry, remove the orphan. "
                    "If it is the only record, re-enter it via PropAgent so it "
                    "generates the correct double-entry pair."
                ),
                'entries': [],
            },
        }]
