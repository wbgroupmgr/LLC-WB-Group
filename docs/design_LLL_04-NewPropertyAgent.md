...
Business Design: `docs/design_BUS_015_NewPropertyAgent.md`  
...
```
ledger/closingAid.py          Core business logic — classification, basis, journal records
ui/llcClosingAid.py           Flask route bindings (/api/closing/...)
ui/templates/_closing_aid_dialog.html   Multi-step modal dialog (Steps 0–4)
ui/templates/table_view.html  Button trigger: "🏠 NewPropertyAgent" in llcAssets Actions menu
tests/test_closingAid.py      40 unit tests covering all core methods
docs/design_BUS_015_NewPropertyAgent.md  Business / accounting spec
```

Internal module names (`closingAid`, `_closing_aid_dialog`, API prefix `/api/closing/`) are unchanged — the rename is UI-only.
...
```
User: "🏠 NewPropertyAgent" button (llcAssets Actions menu)
        ↓
_closing_aid_dialog.html   multi-step modal (Steps 0–4)
        ↓ fetch POST
ui/llcClosingAid.py         route bindings (bind_closing_routes)
        ↓
ledger/closingAid.py        ClosingAid class — pure logic, no Flask dependency
        ↓ save()
llcAssets manager           writes to LLC-WBGroup/books/2025/Accts/llcAssets_WBGroupLLC.json
```

The `ClosingAid` class (`ledger/closingAid.py`) is stateless and Flask-free — all state lives in the dialog's JavaScript IIFE.

---

## 3. Core Class — `ledger/closingAid.py`
...
31 `(keyword_lower, tax_bucket, acct)` tuples. First match wins. More-specific keywords precede broader ones (e.g. `'settlement or closing fee'` before `'title'`). Validated against the 805 High Mesa ALTA statement (`ccDict` in `20250820-ClosingToLedger.ipynb`).
...
Raises `ClosingBalanceError` if not balanced. Calls `_apply_land_split` when `landPct > 0`. Produces one record per row in the llcAssets schema:
...
| `refDB` | `'closingAid'` | Origin marker |
...
## 4. API Routes — `ui/llcClosingAid.py`

All routes registered via `bind_closing_routes(app, objects, sanitize)` called from `ui/llcMgmt._bind_routes()`.

| Route | Method | Request Body | Response |
|---|---|---|---|
| `/api/closing/classify` | POST | `{rows, session_rules?}` | `{ok, classified}` |
| `/api/closing/balance_sheet` | POST | `{classified}` | `{ok, total_debits, total_credits, balanced, delta}` |
| `/api/closing/property_basis` | POST | `{classified, landPct?}` | `{ok, gross_basis, basis_rows, land_amt?, bldg_amt?, land_pct?, bldg_pct?}` |
| `/api/closing/balance_assist` | POST | `{classified, closingDate}` | `{ok, balanced, delta, gl_context, total_funded, covers_delta, suggestion?}` |
| `/api/closing/check_existing` | POST | `{classified, closingDate}` | `{ok, matches}` |
| `/api/closing/commit` | POST | `{classified, preface}` | `{ok, committed, total_records}` — 422 on imbalanced |
...
## 5. Dialog — `ui/templates/_closing_aid_dialog.html`
...
| **1 — Input** | Paste Statement | Textarea (JSON array or CSV); POST `/api/closing/classify`; parse errors shown inline |
...
| **3 — Balance Check** | Balance Assist | POST `/api/closing/balance_assist`; shows GL funding context + ΣDebits vs ΣCredits; `+Add` suggestion button; BALANCED gate before Commit |
| **4 — Basis & Commit** | Property Basis | POST `/api/closing/property_basis`; read-only basis table; land/building split preview; Commit button → POST `/api/closing/commit` |
...
3. On dialog close (`caClose()`): if `_committed` → `window.location.reload()` to show new records in the asset table

### 5.3 Key JavaScript Functions

| Function | Purpose |
|---|---|
| `closingAidOpen()` | Reset state, show backdrop |
| `caClose()` | Hide backdrop; reload page if committed |
| `caGoTo(step)` | Advance/retreat to step; trigger API calls |
| `caClassify()` | Step 1 → POST classify; populate review table |
| `caFlash(msg)` | Show/clear status message |
| `caPost(url, body)` | Thin `fetch` wrapper using `_scriptRoot` prefix |
| `caGoToInput(target)` | Used by unmatched rule-panel Back button |
...
`Ledger = 'nan'` on every ClosingAid record. Two code paths handle this:
...
Result: a ClosingAid record produces exactly 1 GL row (the `acct` side). A regular dual-entry record produces 2 GL rows. Existing behavior unchanged.
...
| `landPct` | float 0–100 | No | Tax assessor land % of total assessed value |
...
| `refDB` | string | Auto | Always `'closingAid'` |
| `refDoc` | string | Auto | `f"{propNm}, Closing Docs, {tax_bucket}, {closingDoc}"` |

---

## 9. Test Coverage — `tests/test_closingAid.py`

40 tests, all passing. Key coverage areas:

| Area | Tests |
|---|---|
| Classification | ALTA Buyer/Seller format, standard Debit/Credit, null-row skipping, Totals row skipping, all 31 rules, fallback `_matched=False` |
