...
class FormWorksheetCmd:
    ...
    def run(self) -> Dict:
        """Execute all steps and return the namespace dict."""
        if self.verbose:
            print(f"\n{'═'*64}")
            print(f"  FormWorksheetCmd  →  {self.form_filename}")
            print(f"{'═'*64}")

        if not self.irs_pdf_path.exists():
            raise FileNotFoundError(f"IRS PDF not found: {self.irs_pdf_path}")

        if self.verbose:
            print(f"  Loading fields from {self.form_filename} …")
        irs_fields = self._load_irs_fields()
        if self.verbose:
            print(f"    {len(irs_fields)} AcroForm fields discovered")

        if self.verbose:
            print(f"  Loading key map from {self.keys_pdf_path.name} …")
        key_map = self._load_key_map()
        if self.verbose:
            print(f"    {len(key_map)} logical-key mappings loaded")

        if self.verbose:
            print(f"  Loading labels from {self.fnames_path.name} …")
        label_map = self._load_label_map()
        if self.verbose:
            print(f"    {len(label_map)} field labels loaded")

        if self.verbose:
            print(f"  Building namespace …")
        namespace, fid_to_pdf = self._build_namespace(irs_fields, key_map, label_map)

        if self.verbose:
            print(f"  Saving namespace JSON …")
        self._save_json(namespace)

        if self.verbose:
            print(f"  Filling and saving namespace PDF …")
        self._save_pdf(namespace, fid_to_pdf)

        if self.verbose:
            # Print a summary by page and type
            print(f"\n  Field summary:")
            page_type: Dict[int, Dict[str, int]] = {}
            for fd in namespace["fields"].values():
                pg    = fd["page"]
                ftype = fd["fType"]
                page_type.setdefault(pg, {})
                page_type[pg][ftype] = page_type[pg].get(ftype, 0) + 1
            for pg in sorted(page_type):
                dist = page_type[pg]
                parts = ", ".join(f"{t}={n}" for t, n in sorted(dist.items()))
                print(f"    Page {pg}: {parts}")
            print(f"\n  Total: {namespace['total_fields']} fields")
            print(f"{'═'*64}\n")
...
def main():
    form_filename = sys.argv[1] if len(sys.argv) > 1 else "Form_1065-IRS.pdf"
    cmd = FormWorksheetCmd(form_filename, verbose=True)
    cmd.run()
