'''
llcPropertyReport — per-property transaction report.
...
Column spec (matches user request):
...
class llcPropertyReport:
    ...
    def _owner_names_str(self, prop_owners: Any) -> str:
        '''
        Resolve propOwners dict → comma-joined owner names.
        propOwners = {"oID": integer_pct, ...}
        '''
    ...
    def _tID_extension(tID: str) -> str:
        '''
        Extract the extension from a tID of the form "<prefix>-<extension>".
        e.g. "a20250826-Cash1" → "Cash1"
        Falls back to the full tID if there is no "-" separator.
        '''
        s = str(tID or '')
        idx = s.rfind('-')
    ...
    def _build_report(self) -> List[Dict[str, Any]]:
        ...
        # ── group records by propID (preserve insertion order within each group)
        ...
        for pid, recs in groups.items():
            # ── build property identity string from first record in group
            first = recs[0]
            ...
            # ── compute balance for this property (Debit sum − Credit sum)
            total_debit  = 0.0
            total_credit = 0.0
            for rec in recs:
                try:
            ...
            # ── detail rows — one per source record
            for rec in recs:
                try:
                    amt = float(rec.get('amt', 0) or 0)
                except (TypeError, ValueError):
                ...
                rows.append({
                    'row_type':     'data',
                    'propID':       pid,
                    'prop_identity': '',          # blank for detail rows
                    'prop_nm':      '',
                    'prop_addr':    '',
                    'prop_owners':  '',
    ...
    def load(self, view_by: str = 'All') -> List[Dict[str, Any]]:
        ...
        if view_by and view_by != 'All':
            # Filter: show only header+details for matching propID or propNm
            target = view_by.lower()
            keep_pids: set = set()
            for r in rows:
                if r['row_type'] == 'property-header':
                    identity = r.get('prop_identity', '').lower()
                    pid      = r.get('propID', '').lower()
                    if target in identity or target in pid:
    ...
    def meta(self) -> Dict[str, Any]:
        return {
            'objectName': 'llcPropertyReport',
            'note':       'Read-only. Per-property transaction report from llcAssets.',
        }

    def object_name(self) -> str:
        return 'llcPropertyReport'
